...
- [x] **[code]** **Completion/distinction aligned to the manual** (`..._completion_manual.sql`): written+practical gates, Silver+/Gold skill gates, attendance ≥ 90%, game-protection failures. Snapshot cites `TGI Manual v1`.
- [x] **[code]** **Practical-exam instrument** (`..._practical_exam.sql`, `content/practical_exam_2026_1.sql`, `PracticalExamScreen`): the Ch 25 nine categories scored 1–5, server-computed composite (≥ 80%), automatic-failure override. Feeds `evaluate_completion()`.
- [x] **[code]** Instructor manual-grading UI for the **written** exam (`WrittenGradingScreen`) + submit-ungraded flow; `assessments.grading` mode drives auto-vs-manual.
- [x] **[code]** Appendix C **Hand Reading Workbook** — 50 hands loaded as a self-check flashcard deck (Ch 20), surfaced in the student Practice hub.
- [x] **[code]** Appendix A **Glossary** (21 terms) + Appendix B **Quick Reference Charts** (Hand Rankings, Critical Rules, Performance Standards, Shuffle Sequence, Dealer Priority) loaded as study flashcards; glossary surfaced in the Practice hub.
- [ ] **[institute]** **Review/clean the workbook answer key** — several source entries contain the author's working-out with self-corrections ("A wins … B wins."); reproduced verbatim, not machine-normalized. (This is also why it's a self-check deck, not an auto-graded quiz.)
- [ ] **[decision — earlier flag, now largely resolved in code]** Original completion/distinction reconciliation — The manual's *Program Completion Requirements* (pass written; **practical ≥ 80% composite**; **Silver+** in all timed/accuracy skills; competency in each game variant; attendance **≥ 90%**) and *Distinction* (**93%+ on BOTH written AND practical**, Gold in all skills, **no game-protection failures**, perfect attendance) are **stricter/broader than the implemented rule** (which follows the original spec: written ≥ 93, all-gold, perfect attendance, full hours). Aligning to the manual needs a modeled **practical-exam score** and a **game-protection-failure** signal, plus an edit to `evaluate_completion()`. Do NOT change silently — ratify with the institute/TWC first.
